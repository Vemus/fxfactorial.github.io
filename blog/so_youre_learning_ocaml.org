...
1) Compilers!, lots of compilers/compiler tools are written in
   OCaml: Facebook uses OCaml for [[https://github.com/facebook/pfff][pfff]] and [[https://github.com/facebook/flow][flow]] and the first cut of
   Rust was written in OCaml.
2) Financial world, [[https://www.janestreet.com][Jane Street]] uses OCaml for basically everything (AFAIK)
3) Systems Programming: [[https://ahrefs.com][ahrefs]], my employer, uses OCaml for heavy
   systems programming.
4) Kernels: Unikernels are hot right now, the most prominent one is
   the [[https://mirage.io][Mirage-OS]] project and its all OCaml.
5) Shameless plug: I use OCaml as well for ~js_of_ocaml~, in fact I'm
   ...
   OCaml compiled into JS, then run on node/Electron), see [[https://github.com/fxfactorial/ocaml-electron][here.]]
6) Genomics/Bioinformatics: [[https://github.com/hammerlab][Hammer Lab]] in NYC uses OCaml for their
