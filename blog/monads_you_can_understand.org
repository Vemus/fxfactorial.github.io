...
#+BEGIN_SRC shell
$ opam installl lwt cohttp.lwt yojson
#+END_SRC
These are the libaries we will use: ~lwt~ is for threading, ~cohttp.lwt~
is for our HTTP requests and ~yojson~ is an easy to use json library.
...
#+BEGIN_SRC ocaml
type t = { authed_headers: Cohttp.Headers.t; end_point : string; }
val make_handle : unit -> t
val create_charge : int -> string -> t -> Yojson.Basic.json Lwt.t
#+END_SRC
...
other wise boilerplate code we'd have to do instead.
